...
public class Play2 extends AsyncTask<Musica,Void,Void>{
    ...
    public Play2(Context context, PlayContract.Actions pa) {
...
/*        while (!finished){
        ...
        while (!finished){
            for (i = 0; i < musica.getCompassos().size(); i++){
                ...
                timer.scheduleAtFixedRate(new Play2.Click(),0,intervalo);
                paused = true;
                synchronized (pauseLock){
                    while (paused){
                        try {
